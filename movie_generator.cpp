...
MovieGenerator::MovieGenerator(int width, int height, std::string color_mode)
    : width(width),
      height(height),
      framesDir("frames"),
      dataDir("data"),
...
void MovieGenerator::generateMovie(const std::string &simName, int fps)
{
    ...
    std::string outputVideo = videoDir + "/" + simName + "_" + color_mode + ".mp4";
...
void MovieGenerator::readCellDataFromFile(const std::string &filename)
{
    ...
    while (std::getline(file, line))
    {
        ...
        // Keep track of unique clone_ids
        if (clone_colors.find(cell.clone_id) == clone_colors.end()) {
            // Assign a random color to this clone_id
            if (cell.clone_id == -1) {
                clone_colors[cell.clone_id] = sf::Color(128, 128, 128); // Gray color
            } else {
                clone_colors[cell.clone_id] = sf::Color(rand() % 256, rand() % 256, rand() % 256);
            }
        }
    ...
    for (const auto &[step, cells] : stepData)
    {
        ...
        for (const auto &cell : cells)
        {
            if (cell.active)
            {
                ...
                sf::Color cellColor;
                if (color_mode == "clone") {
                    cellColor = clone_colors[cell.clone_id];
                } else {
                    const auto &color = CELL_COLORS.at(cell.type);
                    cellColor = sf::Color(std::get<0>(color), std::get<1>(color), std::get<2>(color));
                }
